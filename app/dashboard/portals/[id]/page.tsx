...
export default function EditPortalPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!formData.name.trim()) {
      validationErrors.push({ field: 'name', tab: 'Portal Details', message: 'Portal name is required' })
    }
    
    if (!formData.storageProvider || !formData.storageFolderId) {
      validationErrors.push({ field: 'storage', tab: 'Storage & Files', message: 'Storage provider and folder must be selected' })
    }
    
    if (!formData.maxFileSize || Number(formData.maxFileSize) <= 0) {
      validationErrors.push({ field: 'maxFileSize', tab: 'Access & Security', message: 'Maximum file size must be specified' })
    }
                              ...
                              <div>
                                ...
                                <div className="mt-3 pt-3 border-t border-warning/20">
                                  ...
                                  <p className="text-xs text-muted-foreground mt-2">
                                    Visit <strong>Settings → Connected Accounts</strong> to fix these issues, or use the Storage Health Check tool.
                                  </p>
                                </div>
                              </div>
                            </div>
